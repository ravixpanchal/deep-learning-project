...
parser.add_argument('--save', default='./trainSoftmaxOnly/', type=str, metavar='PATH',
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with open(args.save+"/resnet34_finetunetxt_softmaxonly.txt", 'a+') as textfile:
...
def save_checkpoint(state, is_best, filepath, actuals, probabilities, newmodel, val_loader, epochn, name='finetunedTL_checkpoint_softmaxonly.pth.tar'):
    torch.save(state, os.path.join(filepath, name))
    if is_best:
        shutil.copyfile(os.path.join(filepath, name), os.path.join(filepath, 'finetunedTL_softmaxonly.pth.tar'))
